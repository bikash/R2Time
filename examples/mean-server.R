...
metric = 'cipsi.haisen.proc.loadavg.1m' ## Assign multiple metrics
startdate ='2012/11/07-09:00:00' ## Start date and time of timeseries
...
enddate ="2012/11/07-10:00:00"

outputdir = "/home/bikash/tmp/mean/ex1.1" ## Output file, should be in HDFS file system.
jobname= "Calculation of mean for Data points" ## Assign relevant job description name.
...
reduce <- expression(
   ...
   reduce={
      avg <- avg + mean(sapply(reduce.values, function(x) mean(x)))
   },
   post={
      rhcollect(reduce.key, avg)
   }
)
